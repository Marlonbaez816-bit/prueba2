...
window.AuraAI = (() => {
  ...
  const commands = [
    ...
    { match:['ayuda','qué puedes hacer','comandos','qué sabes hacer'],
      action:()=>{ reply('Puedo navegar entre páginas, cambiar el tema, crear Ecos, abrir la galería, manejar notificaciones y mucho más. Solo dímelo.'); } },
        ...
        <div style="display:flex;flex-direction:column;gap:14px;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            ${[
              ...
              ['🔍','Buscar',        "AuraRouter.navigate('search')"],
              ['🖼️','Galería',       "AuraRouter.navigate('media')"],
              ['🔔','Notificaciones',"AuraRouter.navigate('notifications')"],
              ['🎨','Temas',         "AuraModal.show({title:'Elige tu tema',content:AuraTheme.renderPicker()})"],
              ['✏️','Crear Eco',     "AuraCreate?.open('text')"],
              ['⚙️','Asistente',    "AuraAI.openSettings()"],
  ...
  function toggleEnabled() {
    ...
    const sub = document.getElementById('ai-toggle')
      ?.closest('.settings-row')
      ?.querySelector('.settings-row__subtitle');
    if (sub) sub.textContent = isEnabled
