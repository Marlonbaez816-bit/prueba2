window.AuraCamera = (() => {
    ...
    page.innerHTML = `
      ...
      <div class="camera-controls">
        <div class="camera-filters" id="cam-filters">
          ${filters.map((f, i) => `
            <div class="camera-filter-btn ${i === 0 ? 'active' : ''}" onclick="AuraCamera._setFilter('${f.value}', this)">
              <div class="camera-filter-preview" style="filter:${f.value}"></div>
        ...
        <div class="camera-actions">
          ...
          <div style="display:flex;flex-direction:column;align-items:center;gap:4px">
            <button class="camera-side-btn" id="cam-timer-btn" onclick="AuraCamera._cycleTimer()" title="Temporizador">
              ⏱
            </button>
          ...
          <!-- Flip cámara -->
          <div style="display:flex;flex-direction:column;align-items:center;gap:4px">
            <button class="camera-side-btn" onclick="AuraCamera._flip()" title="Girar cámara">
              🔄
            </button>
  ...
  async function _startStream() {
    try {
      ...
      AuraToast.show('No se pudo acceder a la cámara 📷', 'error');
      close();
    }
  }
  ...
  function _showPreview(src) {
    const existing = document.getElementById('cam-preview-overlay');
    ...
    overlay.innerHTML = `
      <img src="${src}" alt="Foto" style="max-width:90%;max-height:60dvh;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.5);animation:bounceIn .4s cubic-bezier(.34,1.56,.64,1)">
      ...
      AuraModal.show({
        ...
        content: `
          <div style="display:flex;flex-direction:column;gap:14px;">
            <img src="${lastPhoto}" style="width:100%;border-radius:14px;max-height:260px;object-fit:cover;">
            <textarea id="post-caption" placeholder="Escribe algo..." rows="3"
              style="width:100%;background:var(--color-bg-input);border:1px solid var(--color-border);
              border-radius:12px;padding:12px;font-size:15px;color:var(--color-text-primary);
  ...
  function _confirmPost() {
    ...
    // Aquí conectarás con Firestore cuando lo integres
  ...
  function _showVideoPreview(url, blob) {
    const existing = document.getElementById('cam-preview-overlay');
    if (existing) existing.remove();
    ...
    overlay.innerHTML = `
      <video src="${url}" controls autoplay muted loop
        style="max-width:90%;max-height:60dvh;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.5)"></video>
      <div class="camera-preview-actions">
        <button onclick="AuraCamera._discardPhoto()"
          style="background:rgba(255,255,255,.15);backdrop-filter:blur(8px);color:white;
          border:1px solid rgba(255,255,255,.3);padding:12px 24px;border-radius:99px;
  ...
  function _postVideo(url) {
    ...
    setTimeout(() => {
      AuraModal.show({
        ...
        content: `
          <div style="display:flex;flex-direction:column;gap:14px;">
            <video src="${url}" controls muted style="width:100%;border-radius:14px;max-height:220px;object-fit:cover;"></video>
            <textarea id="post-caption" placeholder="Escribe algo..." rows="3"
              style="width:100%;background:var(--color-bg-input);border:1px solid var(--color-border);
              border-radius:12px;padding:12px;font-size:15px;color:var(--color-text-primary);
              resize:none;font-family:inherit;"></textarea>
  ...
  function _initZoom() {
    ...
    if (!vf) return;
    let initialDist = 0, currentZoom = 1;
    ...
    vf.addEventListener('touchmove', e => {
      ...
      const video = document.getElementById('cam-video');
      if (video) video.style.transform = `scaleX(${facingMode === 'user' ? -1 : 1}) scale(${currentZoom})`;
